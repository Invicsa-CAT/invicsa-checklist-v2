...
const SECCIONES = [
  {
    title: '7. Finalización y cierre',
    items: [
      { code: '7.1.1.1', label: 'Operaciones cerradas según ATSP' },
      { code: '7.1.2.1', label: 'Comunicación finalización a terceros' },
      { code: '7.1.2.2', label: 'Comunicación imágenes ZRVF al CECAF' }
    ]
  },
  {
    title: '7.2 Registro de datos de vuelo',
    items: [
      { code: '7.2.1.1', label: 'Anotación tiempos de vuelo' },
      { code: '7.2.1.2', label: 'Anotación tiempos actividad personal' },
      { code: '7.2.2.1', label: 'Anotación eventos de seguridad' },
      { code: '7.2.2.2', label: 'Comunicación incidentes a CIAIAC' }
    ]
  },
  {
    title: '7.2.3 Aeronave',
    items: [
...
export default function Apendice6Page({ op, onBack, onSigned }) {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        if (draft) setState({ ...draft, firmanteName: draft.firmanteName || user?.nombre_completo || '' });
      } catch (e) {
        setError(e.message);
      } finally {
        setLoading(false);
  ...
  async function handleSign() {
    ...
    setSaving(true);
    try {
      await api.signApendice(op.id, '6', {
        items: state.items,
  ...
  return (
    <div className="min-h-screen pb-24">
      <Header
        title="Apéndice 6 — Verificación postvuelo"
